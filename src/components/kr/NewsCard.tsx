...
import { NewsPost } from '@/data/kr/news';
...
export default function NewsCard({ title, date, summary, image, slug }: NewsPost) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow hover:shadow-lg overflow-hidden transition-all">
      ...
      <div className="p-4">
        <Link href={withLocale(`/news/${slug}`)}>
        ...
        <Link href={withLocale(`/news/${slug}`)} className="inline-block mt-4 text-[#40937e] hover:underline text-sm">
          Read More &rarr;
        </Link>
      </div>
    </div>
